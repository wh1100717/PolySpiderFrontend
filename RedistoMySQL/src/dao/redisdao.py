...
from src.util import ComminUtil

redis_client = RedisUtil.RedisClient()

#获取app信息，传入起始和终止数，返回一个list
def get_app_info(startnums,endnums):
    ...
    for app1 in app_lists:
        ...
        if flag==True:
            ...
            app_desc = 'null'
            #获取download_times
            app_download_times=app1_detail['download_times'].encode('utf8', 'ignore')
            #获取cover
            cover=app1_detail['cover']
            #获取creat_time
            ...
            #下载app_icon_file
            ComminUtil.download_icon(cover,app_name)
        flag=False
    return app_infos
